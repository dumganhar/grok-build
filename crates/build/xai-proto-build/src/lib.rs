...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // Use temp files instead of /dev/stdout and /dev/null so this works on Windows
        // (those Unix paths do not exist there; CI falls back to PATH protoc).
        for proto in protos {
            let tmp = tempfile::TempDir::new().context("create temp dir for protoc deps")?;
            let dep_out = tmp.path().join("deps");
            let desc_out = tmp.path().join("out.pb");
            ...
            // Makefile-style: "<descriptor_path>: <dep> <dep> ..."
            // Use ": " (not bare ':') so Windows drive letters (C:\) do not break parsing.
            let rem = first_line
                .split_once(": ")
                .map(|(_, deps)| deps)
                .with_context(|| {
            ...
            for line in iter::once(rem).chain(lines) {
                ...
                // Also match Windows-style separators from some protoc builds.
