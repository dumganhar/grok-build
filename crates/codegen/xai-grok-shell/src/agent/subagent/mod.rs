...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    reconcile_orphaned_subagents_with_backend_mode(
        unfinished,
        backend,
        session_dir,
        parent_session_id,
...
async fn reconcile_orphaned_subagents_with_backend_mode(
    unfinished: &[(String, String)],
    backend: &xai_grok_tools::implementations::grok_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    ...
    if let Ok(entries) = std::fs::read_dir(&subagents_dir) {
        for entry in entries.flatten() {
            ...
            } else if replay_terminal_todos && !candidates.contains_key(&meta.subagent_id) {
                restore_subagent_todo_binding(
                    &meta.subagent_id,
                    meta.todo_id.as_deref(),
                    meta.todo_generation,
