...
impl FinalizedToolset {
    ...
    ///
    /// Unlike `flush_persistence()`, which only flushes previously queued snapshots, this takes a
    ...
    pub async fn try_save_and_flush_persistence(
        &self,
    ) -> std::io::Result<Option<&std::path::Path>> {
